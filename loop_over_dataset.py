...
model = "fpn-resnet" # darknet
model_alias = "resnet" # darknet
sequence = "1"
results_fullpath = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'results/' + model + '/results_sequence_' + sequence + '_' + model_alias)
...
configs_det.min_iou = 0.5 # added for ID_S4_EX1
...
exec_visualization = ['pcl_from_rangeimage', 'show_detection_performance'] # options are 'show_range_image', 'show_bev', 'show_pcl', 'show_labels_in_image', 'show_objects_and_labels_in_bev', 'show_objects_in_bev_labels_in_camera', 'show_tracks', 'show_detection_performance', 'make_tracking_movie'
...
while True:
    try:
        ...
        if 'measure_detection_performance' in exec_list:
            ...
            #det_performance = load_object_from_file(results_fullpath, data_filename, 'det_performance_' + configs_det.arch.split('_') + '_' + str(configs_det.conf_thresh), cnt_frame)   
            det_performance = load_object_from_file(results_fullpath, data_filename, 'det_performance_' + 'fpn_resnet' + '_' + str(configs_det.conf_thresh), cnt_frame)   
